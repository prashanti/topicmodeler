function yyyymmdd=extractDatePT(dates);
...
for i=1:length(dates)
    ...
    if ~isempty(S)
        ...
        if ~isempty(S)
            try
                ...
                error('extractDatePT: error[2] parsing data for document %d: ''%s''\n',i,dates{i});
            end
            ...
            if ~isempty(S) 
                ...
                try
                    ...
                    error('extractDatePT: error[3] parsing data for document %d: ''%s''\n',i,dates{i});
                end
            else % test 'yyyy ...'
                S=regexp(dates{i},...
                ...
                if ~isempty(S)
                    ...
                    if verbose 
                        fprintf('extractDatePT: error[4] parsing data for document %d: ''%s''\n',i,dates{i});
                    end
                end
            end
        end
    ...
    if ((yyyymmdd(i,1)>=0 && (yyyymmdd(i,1)<1950 || yyyymmdd(i,1)>2020)) ||...
