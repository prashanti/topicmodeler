...
// Syntax
//   [ WP , DP , Z , X ] = GibbsSamplerLDAExceptions( WS , DS , T , N , ALPHA0 , ALPHA1 , BETA , SEED , OUTPUT , ZIN , XIN )

// WORK ON ALPHAS TO STEER PRIOR


void GibbsSampler( double ALPHA0, double ALPHA1, double BETA, int W, int T, int D, int NN, int OUTPUT, int n, int *z, int *x, int *d, int *w, int *wp, int *dp, int *ztot, int *xcounts0, int *xcounts1, int *order, double *probs, int startcond )
{
  ...
  for (iter=0; iter<NN; iter++) {
      ...
      for (ii = 0; ii < n; ii++) {
          ...
          totprob = (double) 0;
          for (j = 0; j < T; j++) {
              //probs[j] = ((double) GAMMA0  + (double) xcounts0[ di ]) * 
              //                  ((double) wp[ wioffset+j ] + (double) BETA)/( (double) ztot[j]+ (double) WBETA)*( (double) dp[ dioffset+ j ] + (double) ALPHA);
              
              probs[j] = ((double) wp[ wioffset+j ] + (double) BETA)/( (double) ztot[j]+ (double) WBETA)*( (double) dp[ dioffset+ j ] + (double) ALPHA0 );
              
              totprob += probs[j];
          }
          
          // CALCULATE PROB. FOR SPECIAL TOPIC
          //probs[T] = ( (double) GAMMA1 + (double) xcounts1[ di ] ) * 
          //                    ((double) wp[ wioffset + T + di ] + (double) BETA)/( (double) ztot[ T + di ]+ (double) WBETA)*( (double) dp[ dioffset+ T ] + (double) ALPHA);
          
          probs[T] = ((double) wp[ wioffset + T + di ] + (double) BETA)/( (double) ztot[ T + di ]+ (double) WBETA)*( (double) dp[ dioffset+ T ] + (double) ALPHA1 );
                 ...
                 const mxArray *prhs[])
{
  ...
  int *z,*d,*w, *x, *xcounts0, *xcounts1, *order, *wp, *dp, *ztot;
  ...
  ALPHA0 = (double) mxGetScalar(prhs[4]);
  if (ALPHA0<=0) mexErrMsgTxt("ALPHA0 must be greater than zero");

  ALPHA1 = (double) mxGetScalar(prhs[5]);
  if (ALPHA1<=0) mexErrMsgTxt("ALPHA1 must be greater than zero");
  ...
  if (OUTPUT==2) {
      ...
      mexPrintf( "\tHyperparameter  ALPHA0 = %4.4f\n" , ALPHA0 );
      mexPrintf( "\tHyperparameter  ALPHA1 = %4.4f\n" , ALPHA1 );
  ...
  /* run the model */
  GibbsSampler( ALPHA0 , ALPHA1, BETA, W, T, D, NN, OUTPUT, n, z, x, d, w, wp, dp, ztot, xcounts0, xcounts1, order, probs, startcond );
